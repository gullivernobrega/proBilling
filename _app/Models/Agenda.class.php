...
class Agenda {
    ...
    public function ExeDelete($agenda_id) {
        ...
        $read = new Read;
        $read->ExeRead(self::Tabela, "WHERE agenda_id = :id", "id={$this->Agenda_id}");
    ...
    private function setNome() {
        ...
        $readName = new Read;
        $readName->ExeRead(self::Tabela, "WHERE {$Where} agenda_nome = :a", "a={$this->Data['agenda_nome']}");
    ...
    private function Update() {
        $update = new Update;
        $update->ExeUpdate(self::Tabela, $this->Data, "WHERE agenda_id = :id", "id=$this->Agenda_id");

        if ($update->getResult()):
            $this->Result = $update->getResult();
            $this->Erro = array("<b>Sucesso:</b> A Agenda {$this->Data['agenda_nome']} foi alterado no sietema!", KL_ACCEPT);
        endif;
    }
    ...
    private function Delete() {
        $deletar = new Delete();
        $deletar->ExeDelete(self::Tabela, "WHERE agenda_id = :id", "id={$this->Agenda_id}");
