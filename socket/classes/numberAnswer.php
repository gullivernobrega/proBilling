...
//        Pegando Tempo Logado:
        ...
        if (count($arr) == 12) {
            ...
            $numero = explode("=", $arr2[47]);
            $num2 = $numero[1];
            unset($numero);
            $numero = substr($num2, 1);

            //Pegando o nome do Cliente.    
            $nomeCliente = explode("=", $arr2[44]);
            $this->nomeCliente = $nomeCliente[1];

            //Pegando o CPF ou o codigo do cliente.    
            $codigoCliente = explode("=", $arr2[43]);
    ...
    public function EnviaUrl($user, $agents, $arrayAgents) {
        ...
        if ($agents[1] == 'INUSE') {
            if (array_key_exists($user, $arrayAgents) === false) {
                ...
                $url = "http://200.175.139.182/virtua/integracao/enviarMsgDiscador.php?CLIENTE={$agents[3]}&RAMAL={$agents[0]}&NOMEUSUARIO={$agents[0]}&TELEFONE={$agents[2]}&IDLIGACAO=&SAIR=";
                echo "$url \n";
                $iniciar = curl_init();
